...
$notesQuery = mysqli_prepare($conn, "SELECT pdf_id, title, subject, description, date, image_path FROM notes WHERE user_id = ? ORDER BY date DESC");

mysqli_stmt_bind_param($notesQuery, "i", $userId);
mysqli_stmt_execute($notesQuery);

$notesResult = mysqli_stmt_get_result($notesQuery);

while ($row = mysqli_fetch_assoc($notesResult)) {
    $userNotes[] = $row;
}

mysqli_stmt_close($notesQuery);
?>
            ...
            <div class="uploaded-notes">
                <div class="uploaded-notes-header">
                    <div>
                        ...
                        <p>Notes you have uploaded to NoteFlow.</p>
                    </div>

                    <span class="upload-count"><?= count($userNotes) ?></span>
                </div>
                ...
                <?php if (empty($userNotes)): ?>
                    ...
                    <div class="no-uploaded-notes">
                        ...
                        <a href="upload.php" class="upload-note-btn">Upload Note</a>
                    </div>
                        ...
                        <?php foreach ($userNotes as $note): ?>
                            ...
                            <div class="uploaded-note-card">
                                ...
                                <div class="uploaded-note-actions">
                                    ...
                                    <!-- Download -->
                                    <a href="<?= htmlspecialchars($filePath) ?>" class="note-download-btn" download>
