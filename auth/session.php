...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "register") {
        ...
        if ($result->num_rows > 0) {
            header("Location: signup.php?error=Username already exists. Try another username.");
            exit();
        }

        /* Check password */
        if (empty($password)) {
            header("Location: signup.php?error=Password is empty. Try again.");
            exit();
        }

        /* Check password match */
        if ($password !== $repassword) {
            header("Location: signup.php?error=Your password does not match. Try again.");
            exit();
        }
        ...
        if ($save_data->execute()) {
            ...
            $save_data->close();
            header("Location: ../index.php?error=Failed to create account. Try again.");
            exit();
        }
    }
    ...
    if ($action === "login") {
        ...
        /* Check email */
        if (empty($email)) {
            header("Location: login.php?error=Email is empty. Enter your email.");
            exit();
        }
